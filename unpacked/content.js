...
function resolveURLToPath(cUrl, cType, cContent) {
  ...
  if ((foundIndex === -1) || (foundIndex >= 10)) {
    ...
    if (cUrl.indexOf('data:') === 0) {
      ...
      // 是否 需要将 图片分类保存以及将 base64转换成png
      let is_sort = document.getElementById('check-img').checked
      if (is_sort) {
        // 对于 包含 data:image/png 的 base64格式的图 转成png并另外保存到单独的 base64文件夹
        filename = dataURIInfo + '.' + Math.random().toString(16).substring(2) + '.png';
        cUrl = cUrl.replace("data:image/png;base64,", '')
      } else {
  ...
  toDownload.forEach(function (item, index) {
    if (item.getContent && !item.isStream) {
      ...
      item.getContent(function (body, encode) {
        ...
        // 根据 图片分类 设置图片的 encodeing 方式
        let is_sort = document.getElementById('check-img').checked
        if (is_sort) {
          // 对 base64文件 设置 encode 
          if (filename.search(/\.(png|jpg|jpeg|gif|ico|svg)/) !== -1 || filename.search(/data:image\/png/) !== -1) {
            currentEnconding = 'base64';
          } else if (resolvedItem.dataURI) {
        ...
        if (foundIndex === -1) {
          // 是否区分导出文件夹
          if (is_sort) {
            // 区分 文件类型 image 对于 各自不同后缀名（png, jpg, svg, base64转换的png保存到 base64文件夹）的 image 保存到对应后缀的文件夹中 
            if (item.type == "image" && filename.split('.')[filename.split('.').length - 1] !== 'txt') {
              result.push({
                name: filename,
                type: item.type || 'text/plain',
                originalUrl: item.url,
                // 此处的 url 影响文件最终所在的文件夹
                // 修改 base64导出的路径 不影响 png 图片的直接导出到 png文件夹
                url: filename.search("data.image.png.") !== -1 ? `base64/${filename}` : `${filename.split('.')[filename.split('.').length - 1]}/${filename}`, // Actually the path
